...
export default function ExampleProgress({ value = 65 }: { value?: number }) {
  return (
    <Progress.Root className="grid w-full grid-cols-2 gap-y-2" value={value}>
      <Progress.Label className="text-sm font-medium text-white" style={{ fontFamily: 'var(--font-space-mono)' }}>
        13/20 builders are in
      </Progress.Label>
      <Progress.Value className="col-start-2 text-right text-sm text-white" style={{ fontFamily: 'var(--font-space-mono)' }} />
